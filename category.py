...
class categoryClass:
    def __init__(self,root):
        ...
        lbl_name=Label(self.root,text="Enter Category Name",font=("goudy old style",30),bg="white").place(x=50,y=130)
        txt_name=Entry(self.root,textvariable=self.var_name,font=("goudy old style",18),bg="lightyellow").place(x=50,y=210,width=300)

        #====== Button ====

        btn_add=Button(self.root,text="ADD",command=self.add,font=("goudy old style",20),bg="#4caf50",fg="white",cursor="hand2").place(x=360,y=210,width=150,height=35)
        btn_delete=Button(self.root,text="Delete",command=self.delete,font=("goudy old style",20),bg="red",fg="white",cursor="hand2").place(x=520,y=210,width=150,height=35)


        #===Category Details===

        cat_frame=Frame(self.root,bd=3,relief=RIDGE)
        cat_frame.place(x=700,y=100,width=380,height=220)
        ...
        self.lbl_im2.place(x=640,y=328)
    ...
    def add(self):
        ...
        try:
            if self.var_name.get()=="":
                ...
                row=cur.fetchone()
                if row!=None:
                    messagebox.showerror("Error","Category alreday present, try different",parent=self.root)
                else:
